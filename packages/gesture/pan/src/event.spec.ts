...
describe("isPanEvent", () => {
  it("should return true for pan type signal", () => {
    const signal: Signal = { type: "pan", timestamp: 0, deviceId: "" };

    expect(isPanEvent(signal)).toBe(true);
  });

  it("should return false for non-pan signal", () => {
    const signal: Signal = { type: "pointer", timestamp: 0, deviceId: "" };

    expect(isPanEvent(signal)).toBe(false);
  });
});
