import { createStream, type Observer, type Stream } from "cereb";
...
function createTestSource(events: PanEvent[]): Stream<PanEvent> {
  return createStream((observer: Observer<PanEvent>) => {
...
describe("axisLock", () => {
  ...
  describe("cleanup", () => {
    it("should cleanup on unsubscribe", () => {
      ...
      const source = createStream((obs: Observer<PanEvent>) => {
        observer = obs;
      });
