...
): SinglePointerRecognizer<DomSignal<TouchEvent>> {
  function processer(event: DomSignal<TouchEvent>, signal: SinglePointerSignal): boolean {
    const e = event.value;
...
export function singlePointerFromTouch(
  options: SinglePointerOptions = {},
): Operator<DomSignal<TouchEvent>, SinglePointerSignal> {
  return (source) =>
