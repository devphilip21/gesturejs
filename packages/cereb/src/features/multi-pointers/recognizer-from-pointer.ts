...
import type { MultiPointersSignal, PointerInfo } from "./multi-pointers-signal.js";
import {
  createMultiPointersRecognizer,
  type MultiPointersRecognizer,
...
export function createPointerRecognizer(
...
): MultiPointersRecognizer<DomSignal<PointerEvent>> {
  function processor(
    domSignal: DomSignal<PointerEvent>,
    pointerMap: Map<string, PointerInfo>,
    opts: Required<MultiPointersOptions>,
  ...
  return createMultiPointersRecognizer(processor, options);
}

export function multiPointersFromPointer(
  options: MultiPointersOptions = {},
): Operator<DomSignal<PointerEvent>, MultiPointersSignal> {
  return (source) =>
