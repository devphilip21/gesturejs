...
function createDomSignal<E extends Event>(event: E): DomSignal<E> {
  return createSignal("dom", event) as DomSignal<E>;
}

describe("multiPointersFromPointer", () => {
  it("should track two pointers starting", () => {
    const values: MultiPointersSignal[] = [];
    let observer: Observer<DomSignal<PointerEvent>>;

    const source = createStream<DomSignal<PointerEvent>>((obs) => {
      observer = obs;
      return () => {};
    });

    const stream = multiPointersFromPointer()(source);
    stream.on((v) => values.push(v));

    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 1)));
    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 2)));
  ...
  it("should emit snapshot with all active pointers on each event", () => {
    ...
    observer!.next(
      createDomSignal(createMockPointerEvent("pointerdown", 1, { clientX: 10, clientY: 20 })),
    );
    observer!.next(
      createDomSignal(createMockPointerEvent("pointerdown", 2, { clientX: 100, clientY: 200 })),
    );
    observer!.next(
      createDomSignal(createMockPointerEvent("pointermove", 1, { clientX: 15, clientY: 25 })),
    );
  ...
  it("should correctly handle pointer end and removal", () => {
    ...
    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 1)));
    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 2)));
    observer!.next(createDomSignal(createMockPointerEvent("pointerup", 1)));
  ...
  it("should respect maxPointers limit", () => {
    ...
    const stream = multiPointersFromPointer({ maxPointers: 2 })(source);
    stream.on((v) => values.push(v));

    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 1)));
    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 2)));
    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 3)));
  ...
  it("should include individual pointer phase in PointerInfo", () => {
    ...
    observer!.next(createDomSignal(createMockPointerEvent("pointerdown", 1)));
    observer!.next(createDomSignal(createMockPointerEvent("pointermove", 1)));
