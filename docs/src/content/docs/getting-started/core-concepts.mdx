...
<Aside type="tip">
When you need to add computed properties to a Signal, use operators like `offset()` or `withVelocity()`. They extend the Signal's value without mutating the original structure.
</Aside>
...
<Aside type="caution">
This is why Signals should be treated as readonly. The same Signal object may be reused for the next event. Never store references to Signals for later use—extract the values you need immediately.
</Aside>
